...
口径：《产品说明 v3》第 6.3–6.8 节 + 《需求确认记录.md》决议 R10–R14。
- K_m(S_ω) = ∫S_ω·P_m / ∫S_0·P_m（梯形积分，显式传波长轴）；
- D_raw = K_m · f_m(E,t)；符号说明：本文件及优化器沿用 v3 的 I 表示照度，
  与《基材公式补充.docx》中的 E 为同一物理量（lx）；
- D0 通过同一个 raw_damage() 通用函数计算（S0=D55, E0=50 lx, t0=100 h）；
- **pH 已改为各基材的固定属性**，不再是计算变量（公式在该 pH 下成立），
  因此本文件所有函数均不含 pH 参数；界面仅作只读提示；
...
def reference_damage(
    ...
    通过同一个通用 raw_damage() 计算，不手工写死表值。
    """
    return raw_damage(material, wavelength, S_d55, S_d55, I0, t0)
...
def normalized_damage(
    ...
    f = material.f_value(I, t)
    d_raw = K * f
    d0 = reference_damage(material, wavelength, S_d55)
...
def material_detail(
    ...
    K, d_raw, d0, d_norm = normalized_damage(
        material, wavelength, S_candidate, S_d55, I, t
    )
    return {
...
def relic_damage(
    ...
    for m, a in zip(materials, alpha):
        if details:
            row = material_detail(m, wavelength, S_candidate, S_d55, I, t, float(a))
            detail_rows.append(row)
            total += row["alpha_D_norm"]
        else:
            _, _, _, d_norm = normalized_damage(
                m, wavelength, S_candidate, S_d55, I, t
            )
